...
def mysqlData01() : #MySQL 정보 읽기
    con = pymysql.connect(host='192.168.75.129', user='root',
...
def autoData02(): #대용량 CSV --> MySQL DB
    con = pymysql.connect(host='192.168.75.129', user='root',
                          password='1234', db='userDB', charset='utf8')  # 데이터베이스 지정(또는 연결)
    cur = con.cursor()
    # 폴더 선택하고, 그 안의 파일목록들 추출하기.
...
def autoData04(): #MySQL --> DB대용량 CSV
    con = pymysql.connect(host='192.168.75.129', user='root',
                          password='1234', db='userDB', charset='utf8')  # 데이터베이스 지정(또는 연결)
    cur = con.cursor()
...
autoMenu.add_command(label='대용량 CSV --> SQLite', command=autoData01)
autoMenu.add_command(label='대용량 CSV --> MySQL DB', command=autoData02)
autoMenu.add_separator()
autoMenu.add_command(label='SQLite --> 대용량 CSV ', command=autoData03)
autoMenu.add_command(label='MySQL --> DB대용량 CSV', command=autoData04)
window.mainloop()
